...
export class Camera {
  ...
  public moving = (vector: Vector2, isGlobal = false) => {
    this.x_center =
      this.from[0] - (isGlobal ? vector[0] : this.toGlobalScale(vector[0]));
    this.y_center =
      this.from[1] - (isGlobal ? vector[1] : this.toGlobalScale(vector[1]));
  };
  ...
  public toGlobalPosition(pos: Position): Position {
    return [this.x + pos[0] / this.zoom, this.y + pos[1] / this.zoom];
  }
  public toLocalPosition(pos: Position): Position {
    return [(pos[0] - this.x) * this.zoom, (pos[1] - this.y) * this.zoom];
  }
  ...
  public toLocalDimensions(dimensions: Size): Size {
    return [this.toLocalScale(dimensions[0]), this.toLocalScale(dimensions[1])];
  }

  public get x() {
    return this.x_center - this.toGlobalScale(window.innerWidth / 2);
  }
  public get y() {
    return this.y_center - this.toGlobalScale(window.innerHeight / 2);
  }
